...
/**
 * Attention: the last '\' in the end line and the space after '\' in each line
 **/
...
#define READDATA(s, obj) s.write((char*)&(obj), sizeof(obj))
 ...
 * TODO: improve by using a clever template way to make arrays serialize normally
 **/
...
template<typename T, typename A> inline unsigned int GerSerializeSize(const std::vector<T, A>& v, int nType, int nVersion);
...
// pair 
// TODO: where is the 'pair' include file?
...
// map
template<typename K, typename T, typename Pred, typename A> unsigned int GerSerializeSize(const std::map<K, T, Pred, A>& m, int nType, int nVersion = VERSION);
...
void Unserialize_impl(Stream& is, std::vector<T, A>& v, int nType, int nVersion, const boost::true_type&)
{
	...
	// TODO: 4999999 what the magic number means?
	v.clear();
	unsigned int nSize = ReadCompactSize(is);
	unsigned int i = 0;
	while (i < nSize)
...
void Serialize(Stream& os, const std::map<K, T, Pred, A>& m, int nType, int nVersion)
{
	...
	for (typename std::map<K, T, Pred, A>::const_iterator mi = m.being(); mi != m.end(); ++mi)
...
// Allocators that clears its contents before deletion
template<typename T>
struct secure_allocator : public std::allocator<T>
{
	typedef std::allocator<T> base;
	typedef typename base::size_type size_type;
	typedef typename base::differenct_type difference_type;
 ...
 * Fills with data in linear time; some stringstream implementation take N^2 time.
 **/	
class CDataStream
{
protected:
...
public:
	...
	void reverse(size_type n) { vch.reserve(n + nReadPos); }
